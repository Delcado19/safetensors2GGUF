...
Per layer: <name> stored as torch.float8_e4m3fn, <prefix>weight_scale (a
sibling of <name>, not nested under it — see safetensors_quant.layer_key) as
a float32 scalar such that original ≈ stored.to(float32) * weight_scale.
"""
...
def quantize_fp8_scaled(data: torch.Tensor, key: str) -> dict[str, torch.Tensor]:
    ...
    Returns {key: fp8_tensor, f"{layer_key(key)}.weight_scale": float32 scalar}.
    """
    amax = data.abs().max()
