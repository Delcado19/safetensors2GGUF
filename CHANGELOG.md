...
### Docs
- `README.md` / Text-Encoder Conversion: Added a "Known limitation" callout stating that `text_encoder_convert.py`'s `find_convert_script()`/`find_embedded_python()` have no manual-path override (no env var, no GUI Browse) unlike `llama-quantize`'s discovery — they only search ComfyUI-Easy-Install roots. Includes a manual workaround (clone llama.cpp, `pip install -r requirements.txt`, run `convert_hf_to_gguf.py` directly) for users without Easy-Install.
