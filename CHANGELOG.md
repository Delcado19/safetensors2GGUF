...
- **Known remaining limitation — NVFP4 + Lumina2/Z-Image**: independent of the naming bug above, ComfyUI's `model_detection.py` infers Lumina2's `cap_feat_dim` hyperparameter directly from `cap_embedder.1.weight.shape[1]` **before** any dequantization happens. NVFP4 packs 2 values per byte, halving that tensor's on-disk last dimension, so ComfyUI infers the wrong `cap_feat_dim` and then fails to load the (correctly-shaped, untouched) `cap_embedder.0.weight` LayerNorm against a model built with the wrong size. This is a gap in ComfyUI's own shape-based architecture detection (it doesn't special-case quantized/packed tensors), not something this tool's file format can work around without disabling NVFP4 for that one tensor. Not yet fixed here — flagging for a follow-up.
